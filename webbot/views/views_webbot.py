...
#from webbot.views.genai_data_ingestion import get_response_llm,get_claude_llm,data_ingestion,get_vector_store
...
class WebbotWebhook(generics.GenericAPIView):
    ...
    def post(self, request, *arg, **kwargs):
        try:
           # print('post data',request.data['website_user'])
            ...
            print('ganeshhhhhh',self.request_data['userInput'].get('input_message'))
            msg = self.request_data['userInput']
            print("messagemgs------>",msg.get('input_message'))
            self.received_msg = msg.get('input_message') if msg.get('input_message') else msg
            print('self.received_msg',self.received_msg)
            self.process_info()
            ...
            if (not self.user_verified or self.user_verified == False or self.user_verified == "false") and self.website_user:
                ...
                res_ch = rasa_detect_intent(self.received_msg, self.conversation_id, self.bot_id)
                intent_res_ch = res_ch['intent_res']
                df_intent_ch = intent_res_ch['queryResult']['intent']['displayName']
                db_intent_ch = Intent.objects.filter(name=df_intent_ch, bot_id=self.bot_id).first()
                
                if (str(db_intent_ch.name) == str("about_HSI")):
                    print("inside about_HSI")
                    msg = Intent.objects.filter(name="about_HSI", bot_id=self.bot_id).first().response
                else:
                    print("inside-else")
    ...
    def rasa_api(self):
        try:
            ...
            print('db_intent_name-debug',db_intent)
            ...
            if db_intent.response:
                ...
                if not self.user_verified == 'false':
                    response_msg = webbot_response_format(msg=db_intent.response, params=parameters)
                    print('link-response-msg',db_intent.response)
                else:
                    msg = Intent.objects.filter(name="default_user_verify", bot_id=self.bot_id).first().response
                    response_msg = webbot_response_format(msg=msg, params=parameters)
    ...
    def user_info(self, *arg, **kwargs):
        try:
            ...
            print('debug-web-uid',request_body)
            print('web-uidddddd',request_body.get('web_uid'))
            if request_body.get('website_user')==False:
                ...
                print('scripts',request_body.get('web_uid'))
    ...
    #@csrf_exempt
    #@renderer_classes(JSONRenderer) 
    #def GenAI_data_ingestion(self,*arg, **kwargs):
    #    try:
    #        print("inside GenAI_data_ingestion:")
    #        result = data_ingestion()
    #        if isinstance(result, str) and "error" in result.lower():
    #            print("result",result)
    #        else:
    #            print("Data ingestion successful. Number of documents:", result)
    #        get_vector_store(result)
    #        return JsonResponse(status=status.HTTP_200_OK, data="success")
    #    except Exception as e:
    #        print('GenAI_data_ingestion Error:',e)
    #        return JsonResponse(status=status.HTTP_400_BAD_REQUEST, data=e)

    #@csrf_exempt
    #@renderer_classes(JSONRenderer)
    #def genai_response(self, *arg, **kwargs):
    #    try:
    #        user_question=json.loads(self.body)
    #        print("user_question:",user_question)
    #        print("self.bedrock_embeddings",self.bedrock_embeddings)
    #        faiss_index = FAISS.load_local("faiss_index", self.bedrock_embeddings,allow_dangerous_deserialization=True)
    #        print("faiss_index",faiss_index)
    #        llm=get_claude_llm()
    #        response=get_response_llm(llm,faiss_index,user_question['query'])
